    ...
    <InputGroup className={cn("w-auto", className)}>
      <ComboboxPrimitive.Input
        ...
        onKeyDown={(event) => {
          ...
          // Base UI's closed-combobox Escape behavior clears the selected
          // value. Our searchable single-selects are controlled, so that
          // becomes a focus-trapping no-op. Treat the next Escape after the
          // popup closes as dismissing the focused control instead.
...
function ComboboxList({
  ...
  return (
    <ComboboxPrimitive.List
      ...
      className={cn(
        "no-scrollbar scroll-py-1.5 overflow-y-auto overscroll-contain p-1.5 data-empty:p-0",
        className
      )}
      style={
        typeof style === "function"
          ? (state) => ({
              maxHeight:
                "min(15.75rem, calc(var(--available-height) - 2.25rem))",
              ...style(state),
            })
          : {
              maxHeight:
