...
test("opaque preview gestures defer source selection and preserve rendered drags", async () => {
  ...
  try {
    ...
    const calloutFrom = document.indexOf(calloutSource)
    const calloutTo = calloutFrom + calloutSource.length
    ...
    const startSelectionTransitionLog = () =>
      content.evaluate((element) => {
        ...
        const ownerWindow = element.ownerDocument.defaultView as Window & {
          semanticSelectionOriginalDispatch?: (...specs: unknown[]) => void
          semanticSelectionTransitionLog?: {
            anchor: number
            head: number
          }[]
        }
        const log: { anchor: number; head: number }[] = []
        ownerWindow.semanticSelectionTransitionLog = log
        const originalDispatch = view.dispatch.bind(view)
        ownerWindow.semanticSelectionOriginalDispatch = originalDispatch
    ...
    const stopSelectionTransitionLog = () =>
      content.evaluate((element) => {
        ...
        if (view && ownerWindow.semanticSelectionOriginalDispatch) {
          view.dispatch = ownerWindow.semanticSelectionOriginalDispatch
          delete ownerWindow.semanticSelectionOriginalDispatch
        }
      })

    // A drag entering an opaque construct maps its endpoint to the indicated
    // semantic edge and reveals source only on pointerup.
    // Keep this gesture distinct from the preceding synthetic click so
    // Chromium reports a single-click drag rather than a double-click drag.
    await page.waitForTimeout(500)
    const outsideBefore = page
      .locator(".cm-line")
      .filter({ hasText: "Outside before callout." })
    await centerInEditor(callout)
    const outsideBeforeBounds = await outsideBefore.boundingBox()
    const calloutBounds = await callout.boundingBox()
    if (!outsideBeforeBounds || !calloutBounds) {
      throw new Error("Outside-to-callout drag geometry is unavailable")
    }
    await page.mouse.move(
      outsideBeforeBounds.x + outsideBeforeBounds.width - 3,
      outsideBeforeBounds.y + outsideBeforeBounds.height / 2
    )
    await startSelectionTransitionLog()
    await page.mouse.down()
    await page.mouse.move(
      calloutBounds.x + calloutBounds.width / 2,
      calloutBounds.y + calloutBounds.height * 0.2,
      { steps: 5 }
    )
    await page.mouse.move(
      calloutBounds.x + calloutBounds.width / 2,
      calloutBounds.y + calloutBounds.height * 0.8,
      { steps: 5 }
    )
    const includedCalloutDuringDragFromAbove = await selectionSnapshot()
    expect(includedCalloutDuringDragFromAbove.head).toBe(calloutTo)
    expect(includedCalloutDuringDragFromAbove.source).toContain(calloutSource)
    await expect
      .poll(() => callout.locator(".cm-app-selection-foreground").count())
      .toBeGreaterThan(0)
    await page.mouse.move(
      calloutBounds.x + calloutBounds.width / 2,
      calloutBounds.y + calloutBounds.height * 0.2,
    ...
    const startBoundaryCaret = page.locator(
      '.cm-md-semantic-boundary-caret[data-edge="start"]'
    )
    await expect(startBoundaryCaret).toBeVisible()
    await expect(callout).toHaveAttribute("data-semantic-boundary-target", "")
    const clampedSelectionFromAbove = await selectionSnapshot()
    expect(clampedSelectionFromAbove.ranges).toHaveLength(1)
    expect(clampedSelectionFromAbove.anchor).toBe(
      document.indexOf("Outside before callout.") +
        "Outside before callout.".length
    )
    expect(clampedSelectionFromAbove.head).toBe(calloutFrom)
    expect(clampedSelectionFromAbove.source).not.toContain(calloutSource)
    expect(
      clampedSelectionFromAbove.to <= calloutFrom ||
        clampedSelectionFromAbove.from >= calloutTo
    ).toBe(true)
    await expect(callout.locator(".cm-app-selection-foreground")).toHaveCount(0)
    expect(
      await page.evaluate(() => window.getSelection()?.toString())
    ).not.toContain("Callout card")
    const calloutHeadsFromAbove = (await selectionTransitionLog())
      .map((selection) => selection.head)
      .filter((head) => head >= calloutFrom && head <= calloutTo)
    expect(
      calloutHeadsFromAbove.every(
        (head) => head === calloutFrom || head === calloutTo
      )
    ).toBe(true)
    expect(calloutHeadsFromAbove).toContain(calloutTo)
    expect(calloutHeadsFromAbove.at(-1)).toBe(calloutFrom)
    await stopSelectionTransitionLog()
    const startBoundaryBounds = await startBoundaryCaret.boundingBox()
    if (!startBoundaryBounds) {
      throw new Error("Callout start boundary caret is unavailable")
    }
    expect(startBoundaryBounds.height).toBeGreaterThan(
      calloutBounds.height * 0.9
    )
    expect(
      Math.abs(startBoundaryBounds.x + 1 - calloutBounds.x)
    ).toBeLessThanOrEqual(2)
    await page.mouse.up()
    await expect(startBoundaryCaret).toHaveCount(0)
    await expect.poll(selectionSnapshot).toMatchObject({
      empty: false,
      head: calloutFrom,
    })
    const excludedCalloutFromAbove = await selectionSnapshot()
    expect(excludedCalloutFromAbove.ranges).toHaveLength(1)
    expect(excludedCalloutFromAbove.source).not.toContain(calloutSource)
    expect(
      excludedCalloutFromAbove.to <= calloutFrom ||
        excludedCalloutFromAbove.from >= calloutTo
    ).toBe(true)
    ...
    const outsideAfter = page
      .locator(".cm-line")
      .filter({ hasText: "Outside after callout." })
    await centerInEditor(callout)
    const outsideAfterBounds = await outsideAfter.boundingBox()
    const restoredCalloutBounds = await callout.boundingBox()
    if (!outsideAfterBounds || !restoredCalloutBounds) {
      throw new Error("Reverse callout drag geometry is unavailable")
    }
    await page.mouse.move(
      outsideAfterBounds.x + 4,
      outsideAfterBounds.y + outsideAfterBounds.height / 2
    )
    await startSelectionTransitionLog()
    await page.mouse.down()
    await page.mouse.move(
      restoredCalloutBounds.x + restoredCalloutBounds.width / 2,
      restoredCalloutBounds.y + restoredCalloutBounds.height * 0.8,
      { steps: 5 }
    )
    await expect(
      page.locator('.cm-md-semantic-boundary-caret[data-edge="end"]')
    ).toBeVisible()
    const clampedSelectionFromBelow = await selectionSnapshot()
    expect(clampedSelectionFromBelow.ranges).toHaveLength(1)
    expect(clampedSelectionFromBelow.anchor).toBe(
      document.indexOf("Outside after callout.")
    )
    expect(clampedSelectionFromBelow.head).toBe(calloutTo)
    expect(clampedSelectionFromBelow.source).not.toContain(calloutSource)
    expect(
      clampedSelectionFromBelow.to <= calloutFrom ||
        clampedSelectionFromBelow.from >= calloutTo
    ).toBe(true)
    await expect(callout.locator(".cm-app-selection-foreground")).toHaveCount(0)
    await page.mouse.move(
      restoredCalloutBounds.x + restoredCalloutBounds.width / 2,
      restoredCalloutBounds.y + restoredCalloutBounds.height * 0.2,
      { steps: 5 }
    )
    const includedCalloutDuringDragFromBelow = await selectionSnapshot()
    expect(includedCalloutDuringDragFromBelow.head).toBe(calloutFrom)
    expect(includedCalloutDuringDragFromBelow.source).toContain(calloutSource)
    await expect
      .poll(() => callout.locator(".cm-app-selection-foreground").count())
      .toBeGreaterThan(0)
    await page.mouse.move(
    ...
    expect((await selectionSnapshot()).source).not.toContain(calloutSource)
    const calloutHeadsFromBelow = (await selectionTransitionLog())
      .map((selection) => selection.head)
      .filter((head) => head >= calloutFrom && head <= calloutTo)
    expect(
      calloutHeadsFromBelow.every(
        (head) => head === calloutFrom || head === calloutTo
      )
    ).toBe(true)
    expect(calloutHeadsFromBelow).toContain(calloutFrom)
    expect(calloutHeadsFromBelow.at(-1)).toBe(calloutTo)
    await stopSelectionTransitionLog()
    await page.mouse.up()
    await expect.poll(selectionSnapshot).toMatchObject({
      empty: false,
    ...
    const excludedCalloutFromBelow = await selectionSnapshot()
    expect(excludedCalloutFromBelow.ranges).toHaveLength(1)
    expect(excludedCalloutFromBelow.source).not.toContain(calloutSource)
    expect(
      excludedCalloutFromBelow.to <= calloutFrom ||
        excludedCalloutFromBelow.from >= calloutTo
    ).toBe(true)
    await expect(page.locator(".cm-cursor")).toHaveCount(1)
    await expect(callout).toHaveCount(0)
    await page.keyboard.press("Escape")
    ...
    await centerInEditor(callout)
    const inclusionStartBounds = await outsideBefore.boundingBox()
    const inclusionCalloutBounds = await callout.boundingBox()
    if (!inclusionStartBounds || !inclusionCalloutBounds) {
      throw new Error("Inclusive callout drag geometry is unavailable")
    }
    await page.mouse.move(
      inclusionStartBounds.x + inclusionStartBounds.width - 3,
      inclusionStartBounds.y + inclusionStartBounds.height / 2
    )
    await page.mouse.down()
    await page.mouse.move(
      inclusionCalloutBounds.x + inclusionCalloutBounds.width / 2,
      inclusionCalloutBounds.y + inclusionCalloutBounds.height * 0.8,
      { steps: 5 }
    )
    await page.mouse.up()
    await expect.poll(selectionSnapshot).toMatchObject({
    ...
    const includedCalloutFromAbove = await selectionSnapshot()
    expect(includedCalloutFromAbove.ranges).toHaveLength(1)
    expect(includedCalloutFromAbove.source).toContain(calloutSource)
    await expect(callout).toHaveCount(0)
    await page.keyboard.press("Escape")
    await expect(callout).toHaveCount(1)
